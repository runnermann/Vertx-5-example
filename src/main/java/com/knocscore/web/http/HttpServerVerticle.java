...
public class HttpServerVerticle extends VerticleBase {
    ...
    // DB Service Proxy
    private com.knocscore.database.rxjava3.UserDatabaseService dbService;
    ...
    private void init() {
        ...
        dbService = com.knocscore.database.UserDatabaseService.createProxy(vertx, BUS_USERDB_QUEUE);
    }
...
//        router.get("/earning-money-as-a-college-student").handler(context ->
//                httpUtility.pageHandler(context, "webroot/templates/pay/make_sales_explanation.ftl"));
//        router.get("/earning-your-knowledge-credibility")
//                  .handler(ctx -> pageHandler(ctx, "/webroot/templates/qr/earning-qrstats.ftl"));
        router.route("/articles/the-knowledge-credibility-score")
                .handler(context -> pageHandler(context, "webroot/templates/blog/knowledge-credibility-score.ftl"));
//        router.route("/articles/improving-hiring-quality-in-2024")
//                .handler(context -> pageHandler(context, "webroot/templates/blog/improving-hiring-quality-in-2024.ftl"));
//        router.route("/articles/unlocking-high-paying-jobs-using-the-knocscore")
//                .handler(context -> pageHandler(context, "webroot/templates/blog/unlocking-high-paying-jobs.ftl"));
//        router.route("/blog-listing")
//                .handler(this::blogsAllPageHandler);
//        router.route("/articles/:id/:endpoint").handler(this::blogPageHandler);
    ...
    private void landingPage(RoutingContext context) {
        if (httpUtility.logCall(context)) {
            httpUtility.deceptiveRequestHandler(context);
        } else {
            Page page = new Page();
...
//    }
    ...
    private String[] splitRowData(JsonObject obj) {
        ...
        try {
            ...
            return httpUtility.getArrayFromJsonArray(arr);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return null;
